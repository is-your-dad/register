...
function validateRecordValues(t, data, file) {
    ...
    Object.entries(data.records).forEach(([key, value]) => {
        ...
        if (["CNAME", "URL"].includes(key)) {
            ...
            if (key === "CNAME") {
                ...
                t.true(value !== `${subdomain}.is-your.dad`, `${file}: ${key} cannot point to itself`);
                t.true(value !== "is-your.dad", `${file}: ${key} cannot point to is-your.dad`);
                ...
                t.false(urlHost === `${subdomain}.is-your.dad`, `${file}: ${key} cannot point to itself`);
            }
        }
    ...
    if (data.redirect_config) {
        ...
        customPaths.forEach((customPath, idx) => {
            ...
            t.false(urlHost === `${subdomain}.is-your.dad`, `${urlMessage} cannot point to itself at index ${idx}`);
        });
    }
}
